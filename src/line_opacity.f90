...
MODULE LINE_OPACITY
  ...
  USE FORWARD_PARAM, ONLY: LINE_ZN, LINE_ION, LINE_L0, EPLOW, LOGGF
  !
  IMPLICIT NONE
  !
  !::::::::::::::::::::::::::::::::::::::::::::::::
  ...
  !------------------------------------------------
  !
  SUBROUTINE OPAC_LINE(INDEX,TEMP,NELEC,NHYD,KLIN,DKLDT_RHO,DKLDT_PG,DKLDPG_TEM)
    !
    IMPLICIT NONE
    ! Input
    ...
    !
    TERM3=N_PI/U_PI!(N_PI/U_PI)
    !
    TERM4=DEXP(-EPLOW(INDEX)*EVOLT/(KBOL*T8))
    !
    TERM5=1.0D0-DEXP(-HPLA*LIGHT/(1.0D-8*LINE_L0(INDEX)*KBOL*T8))
    !
...
! . use of dne/drho_t instead of dne/dpg_t:
      ...
      DTERM3=(DNIRDT_PG-N_PI*DU_PI)/(U_PI)
      !
      ! Derivative of TERM4
      ...
      DTERM3=(DNIRDT_RHO-N_PI*DU_PI)/(U_PI)
      !
      DKLDTEMP_RHO=TERM1*(&
           DTERM2*TERM3*TERM4*TERM5+&
           TERM2*DTERM3*TERM4*TERM5+&
