...
MODULE INPUT_FILE
  ...
  SUBROUTINE NSET_MISC(START_LINE)
    ...
    reading: DO WHILE (START_LINE.LT.INPUT_READ_NUMBER)
      ...
      DUMSIGL=.FALSE.
      CALL SPLIT_READ_LINE_WEIGHTS(LINE, 'LINEAR_SOLVER', 1, DUMSIGL, DUMSP, DUMIP)
      IF (DUMSIGL.EQV..TRUE.) SOLVER_METHOD=DUMIP
      DUMSIGL=.FALSE.
      CALL SPLIT_READ_LINE_WEIGHTS(LINE, 'PERTURBATION_TYPE', 1, DUMSIGL, DUMSP, DUMIP)
      IF (DUMSIGL.EQV..TRUE.) NODE_EXPANSION=DUMIP
      DUMSIGL=.FALSE.
      ...
      CALL SPLIT_READ_LINE_WEIGHTS(LINE, 'VERBOSE', 0, DUMVERB, DUMSP, DUMIP)
...
! Obsolete nlte dep. coef.:        , NUML_DATABASE, FULL_STOKES, HYDRO_TOP
    ...
    !-----------------------------
    SOLVER_METHOD=1
    NODE_EXPANSION=1 ! By default, SIR/NICOLE inversion nodes
    !-----------------------------
    !
    CNT = 0
    !
