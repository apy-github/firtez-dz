...
!.          !
       ...
       !
       DETAI_DBX(PIXEL_INI(L):PIXEL_END(L))=&
           DETAI_DBX(PIXEL_INI(L):PIXEL_END(L))+DETAI_DU(:)*DUUDB*DB_DBX
       DETAQ_DBX(PIXEL_INI(L):PIXEL_END(L))=&
           DETAQ_DBX(PIXEL_INI(L):PIXEL_END(L))+DETAQ_DU(:)*DUUDB*DB_DBX
       DETAU_DBX(PIXEL_INI(L):PIXEL_END(L))=&
           DETAU_DBX(PIXEL_INI(L):PIXEL_END(L))+DETAU_DU(:)*DUUDB*DB_DBX
       DETAV_DBX(PIXEL_INI(L):PIXEL_END(L))=&
           DETAV_DBX(PIXEL_INI(L):PIXEL_END(L))+DETAV_DU(:)*DUUDB*DB_DBX
       DRHOQ_DBX(PIXEL_INI(L):PIXEL_END(L))=&
           DRHOQ_DBX(PIXEL_INI(L):PIXEL_END(L))+DRHOQ_DU(:)*DUUDB*DB_DBX
       DRHOU_DBX(PIXEL_INI(L):PIXEL_END(L))=&
           DRHOU_DBX(PIXEL_INI(L):PIXEL_END(L))+DRHOU_DU(:)*DUUDB*DB_DBX
       DRHOV_DBX(PIXEL_INI(L):PIXEL_END(L))=&
           DRHOV_DBX(PIXEL_INI(L):PIXEL_END(L))+DRHOV_DU(:)*DUUDB*DB_DBX
       !
       ! Dby
       !
       DETAI_DBY(PIXEL_INI(L):PIXEL_END(L))=&
           DETAI_DBY(PIXEL_INI(L):PIXEL_END(L))+DETAI_DU(:)*DUUDB*DB_DBY
       DETAQ_DBY(PIXEL_INI(L):PIXEL_END(L))=&
           DETAQ_DBY(PIXEL_INI(L):PIXEL_END(L))+DETAQ_DU(:)*DUUDB*DB_DBY
       DETAU_DBY(PIXEL_INI(L):PIXEL_END(L))=&
           DETAU_DBY(PIXEL_INI(L):PIXEL_END(L))+DETAU_DU(:)*DUUDB*DB_DBY
       DETAV_DBY(PIXEL_INI(L):PIXEL_END(L))=&
           DETAV_DBY(PIXEL_INI(L):PIXEL_END(L))+DETAV_DU(:)*DUUDB*DB_DBY
       DRHOQ_DBY(PIXEL_INI(L):PIXEL_END(L))=&
           DRHOQ_DBY(PIXEL_INI(L):PIXEL_END(L))+DRHOQ_DU(:)*DUUDB*DB_DBY
       DRHOU_DBY(PIXEL_INI(L):PIXEL_END(L))=&
           DRHOU_DBY(PIXEL_INI(L):PIXEL_END(L))+DRHOU_DU(:)*DUUDB*DB_DBY
       DRHOV_DBY(PIXEL_INI(L):PIXEL_END(L))=&
           DRHOV_DBY(PIXEL_INI(L):PIXEL_END(L))+DRHOV_DU(:)*DUUDB*DB_DBY
       !
       ! Dbz
       !
       DETAI_DBZ(PIXEL_INI(L):PIXEL_END(L))=&
           DETAI_DBZ(PIXEL_INI(L):PIXEL_END(L))+DETAI_DU(:)*DUUDB*DB_DBZ
       DETAQ_DBZ(PIXEL_INI(L):PIXEL_END(L))=&
           DETAQ_DBZ(PIXEL_INI(L):PIXEL_END(L))+DETAQ_DU(:)*DUUDB*DB_DBZ
       DETAU_DBZ(PIXEL_INI(L):PIXEL_END(L))=&
           DETAU_DBZ(PIXEL_INI(L):PIXEL_END(L))+DETAU_DU(:)*DUUDB*DB_DBZ
       DETAV_DBZ(PIXEL_INI(L):PIXEL_END(L))=&
           DETAV_DBZ(PIXEL_INI(L):PIXEL_END(L))+DETAV_DU(:)*DUUDB*DB_DBZ
       DRHOQ_DBZ(PIXEL_INI(L):PIXEL_END(L))=&
           DRHOQ_DBZ(PIXEL_INI(L):PIXEL_END(L))+DRHOQ_DU(:)*DUUDB*DB_DBZ
       DRHOU_DBZ(PIXEL_INI(L):PIXEL_END(L))=&
           DRHOU_DBZ(PIXEL_INI(L):PIXEL_END(L))+DRHOU_DU(:)*DUUDB*DB_DBZ
       DRHOV_DBZ(PIXEL_INI(L):PIXEL_END(L))=&
           DRHOV_DBZ(PIXEL_INI(L):PIXEL_END(L))+DRHOV_DU(:)*DUUDB*DB_DBZ
       !
       ! Dvlos
       !
       DETAI_DVLOS(PIXEL_INI(L):PIXEL_END(L))=&
           DETAI_DVLOS(PIXEL_INI(L):PIXEL_END(L))+DETAI_DU(:)*DUUDVLOS
       DETAQ_DVLOS(PIXEL_INI(L):PIXEL_END(L))=&
           DETAQ_DVLOS(PIXEL_INI(L):PIXEL_END(L))+DETAQ_DU(:)*DUUDVLOS
       DETAU_DVLOS(PIXEL_INI(L):PIXEL_END(L))=&
           DETAU_DVLOS(PIXEL_INI(L):PIXEL_END(L))+DETAU_DU(:)*DUUDVLOS
       DETAV_DVLOS(PIXEL_INI(L):PIXEL_END(L))=&
           DETAV_DVLOS(PIXEL_INI(L):PIXEL_END(L))+DETAV_DU(:)*DUUDVLOS
       DRHOQ_DVLOS(PIXEL_INI(L):PIXEL_END(L))=&
           DRHOQ_DVLOS(PIXEL_INI(L):PIXEL_END(L))+DRHOQ_DU(:)*DUUDVLOS
       DRHOU_DVLOS(PIXEL_INI(L):PIXEL_END(L))=&
           DRHOU_DVLOS(PIXEL_INI(L):PIXEL_END(L))+DRHOU_DU(:)*DUUDVLOS
       DRHOV_DVLOS(PIXEL_INI(L):PIXEL_END(L))=&
           DRHOV_DVLOS(PIXEL_INI(L):PIXEL_END(L))+DRHOV_DU(:)*DUUDVLOS
       !
       ! Dpg
       !
       DETAI_DPGT(PIXEL_INI(L):PIXEL_END(L))=&
